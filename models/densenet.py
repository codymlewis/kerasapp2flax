...
class ConvBlock(nn.Module):
    ...
    def __call__(self, x, train=True):
        x1 = nn.BatchNorm(epsilon=1.001e-5, name=self.name + '_0_bn', use_running_average=not train)(x)
        x1 = nn.relu(x1)
        x1 = nn.Conv(4 * self.growth_rate, (1, 1), use_bias=False, name=self.name + '_1_conv')(x1)
        x1 = nn.BatchNorm(epsilon=1.001e-5, name=self.name + '_1_bn', use_running_average=not train)(x1)
        x1 = nn.relu(x1)
...
class TransitionBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, train=True):
        x = nn.BatchNorm(epsilon=1.001e-5, name=self.name + '_bn', use_running_average=not train)(x)
        x = nn.relu(x)
...
class DenseNet(nn.Module):
    ...
    def __call__(self, x, train=True):
        ...
        x = nn.BatchNorm(epsilon=1.001e-5, name='conv1/bn', use_running_average=not train)(x)
        x = nn.relu(x)
        ...
        x = nn.BatchNorm(epsilon=1.001e-5, name="bn", use_running_average=not train)(x)
        x = nn.relu(x)
